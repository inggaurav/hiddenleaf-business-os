...
Route::middleware(['auth', 'module.status:account'])->prefix('accounting')->name('account-reference.')->group(function () {
    ...
    // Account types / chart of accounts lifecycle
    ...
    // Customer/vendor payment lifecycle and outstanding invoices
    ...
    // Bank transfer lifecycle (existing POST /accounting/bank-transfers remains immediate-post compatibility)
    ...
    Route::get('{type}-categories', [AccountReferenceController::class, 'categories'])->whereIn('type', ['revenue', 'expense'])->name('categories.index');
    Route::post('{type}-categories', [AccountReferenceController::class, 'storeCategory'])->whereIn('type', ['revenue', 'expense'])->name('categories.store');
    Route::get('{type}-categories/{category}/edit', [AccountReferenceController::class, 'editCategory'])->whereIn('type', ['revenue', 'expense'])->name('categories.edit');
    Route::put('{type}-categories/{category}', [AccountReferenceController::class, 'updateCategory'])->whereIn('type', ['revenue', 'expense'])->name('categories.update');
    Route::delete('{type}-categories/{category}', [AccountReferenceController::class, 'destroyCategory'])->whereIn('type', ['revenue', 'expense'])->name('categories.destroy');

    // Direct revenue lifecycle
    ...
    // Credit / debit note detail and immutable approval lifecycle
    ...
    // WorkDo-equivalent Account reporting outcomes
    Route::get('reports/invoice-aging', [AccountReferenceController::class, 'invoiceAging'])->name('reports.invoice-aging');
    Route::get('reports/invoice-aging/print', fn () => abort(404)); // superseded below by explicit route name resolution
    ...
    Route::get('reports/print/{report}', [AccountReferenceController::class, 'printReport'])
        ->whereIn('report', ['invoice-aging', 'bill-aging', 'tax-summary', 'customer-balance', 'vendor-balance'])
        ->name('reports.print');
