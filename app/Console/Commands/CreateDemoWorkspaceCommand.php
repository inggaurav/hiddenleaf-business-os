...
class CreateDemoWorkspaceCommand extends Command
{
    ...
    private const DEMO_MODULES = [
        'core', 'account', 'sales', 'procurement', 'crm', 'lead', 'hrm', 'productservice',
        'pos', 'taskly', 'landingpage', 'helpdesk', 'media', 'communications', 'automations',
        'missions', 'command_center', 'knowledge',
    ];
    ...
    public function handle(TenantProvisioningService $provisioner, DemoDataSeederService $demoData): int
    {
        ...
        try {
            [$user, $workspace, $plan, $created] = DB::transaction(function () use ($email, $companyName, $requestedPassword, $provisioner, &$generatedPassword): array {
                ...
                if (! $user) {
                    $generatedPassword = is_string($requestedPassword) && $requestedPassword !== '' ? $requestedPassword : Str::password(20);
                    $user = User::create([
                ...
                $plan = Plan::firstOrCreate(['name' => 'Demo'], [
                ...
                foreach (['onboarding_completed' => '1', 'onboarding_current_step' => '6', 'mrfox_enabled' => '1'] as $key => $value) {
                    Setting::updateOrCreate(
                        ['scope' => 'workspace', 'scope_id' => $workspace->id, 'key' => $key],
                        ['organization_id' => $workspace->organization_id, 'workspace_id' => $workspace->id, 'value' => $value, 'is_encrypted' => false]
                    );
                }
            ...
            $this->table(['Field', 'Value'], [
                ['Owner', $user->name], ['Email', $user->email], ['Organization', $workspace->organization->name],
                ['Workspace', $workspace->name], ['Workspace ID', (string) $workspace->id], ['Plan', $plan->name],
                ['Modules', implode(', ', self::DEMO_MODULES)], ['CRM leads', (string) ($counts['leads_count'] ?? 0)],
                ['Deals', (string) ($counts['deals_count'] ?? 0)], ['Products', (string) ($counts['products_count'] ?? 0)],
                ['Invoices', (string) ($counts['invoices_count'] ?? 0)], ['Inbox conversations', (string) ($counts['conversations_count'] ?? 0)],
