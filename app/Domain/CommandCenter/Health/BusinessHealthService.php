...
class BusinessHealthService
{
    ...
    public function evaluateHealth(User $user, Workspace $workspace): array
    {
        ...
        if ($isSuperAdmin || $this->permissionService->allows($user, $workspace, 'account.manage')) {
            $totalReceivables = (float) SalesInvoice::where('workspace_id', $wsId)->whereNotIn('status', [0, 3])->sum('total_amount');
            $overdueInvoices = SalesInvoice::where('workspace_id', $wsId)->whereNotIn('status', [0, 3])->where('due_date', '<', today())->get();
            ...
            // 2. Payables Health
            $totalPayables = (float) PurchaseInvoice::where('workspace_id', $wsId)->whereNotIn('status', [0, 3])->sum('total_amount');
            $overdueBills = PurchaseInvoice::where('workspace_id', $wsId)->whereNotIn('status', [0, 3])->where('due_date', '<', today())->get();
