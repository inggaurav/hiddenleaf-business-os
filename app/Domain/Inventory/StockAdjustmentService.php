...
class StockAdjustmentService
{
    ...
    public function adjust(ProductServiceItem $product, Warehouse $warehouse, float $quantity, string $reason, User $actor): StockMovement
    {
        ...
        return DB::transaction(function () use ($product, $warehouse, $quantity, $reason, $actor) {
            ...
            $this->audit->log($actor->id, $product->organization_id, $product->workspace_id, 'inventory.adjusted', 'stock_movement', (string) $movement->id, [
                'product_id' => $product->id,
                'warehouse_id' => $warehouse->id,
                'quantity' => $quantity,
                'balance_after' => $balance,
