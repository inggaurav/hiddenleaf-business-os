    ...
    <AppShell title="Super Admin Control Center" breadcrumbs={[{ label: 'Platform Administration' }, { label: 'Dashboard' }]}>
      <div className="space-y-6">
        <SectionHeader
          ...
          actions={
            <div className="flex items-center gap-2">
              <Link href="/super-admin/companies"><Button variant="outline" size="sm" icon={<Building2 className="w-4 h-4" />}>Companies</Button></Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricCard title="Active Subscriptions" value={metrics.active_subscriptions ?? 0} icon={<CreditCard className="w-5 h-5 text-[var(--text-secondary)]" />} subtitle="Current subscriptions" />
          <MetricCard title="Processed Orders" value={metrics.orders ?? 0} icon={<ShoppingCart className="w-5 h-5 text-[var(--text-secondary)]" />} subtitle="Billing orders" />
            ...
            {shortcuts.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}>
                  <Card level={0} className="h-full hover:border-[var(--border-medium)] transition-colors">
                    <div className="flex items-start gap-3"><div className="w-9 h-9 rounded-lg bg-[var(--text-primary)]/10 border border-purple-500/20 flex items-center justify-center"><Icon className="w-4.5 h-4.5 text-[var(--text-secondary)]" /></div><div><h3 className="text-sm font-bold">{item.name}</h3><p className="text-xs text-[var(--text-tertiary)] mt-1 leading-relaxed">{item.description}</p></div></div>
                  </Card>
                </Link>
              );
            })}
