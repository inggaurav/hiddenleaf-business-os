      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {workspaces.map((workspace: any) => {
            ...
            return (
              <Card key={workspace.id} level={active ? 1 : 0} className={active ? 'border-[var(--border-strong)]' : ''}>
                <div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3 min-w-0"><div className="w-10 h-10 rounded-xl bg-[var(--surface-2)] border border-[var(--border-subtle)] flex items-center justify-center"><Layers className="w-5 h-5 text-[var(--text-secondary)]" /></div><div className="min-w-0"><div className="font-bold text-sm truncate">{workspace.name}</div><div className="text-[11px] text-[var(--text-tertiary)] truncate">{workspace.slug}</div></div></div><Badge variant={active ? 'purple' : (workspace.is_active ? 'success' : 'neutral')} size="sm">{active ? 'Current' : (workspace.is_active ? 'Active' : 'Inactive')}</Badge></div>
                <div className="mt-4 pt-4 border-t border-[var(--border-subtle)] flex items-center justify-between gap-2">
                  {!active ? <Button size="sm" variant="primary" onClick={() => router.post('/workspaces/switch', { workspace_id: workspace.id })}>Switch</Button> : <span className="text-[11px] text-[var(--text-secondary)]">Current workspace</span>}
