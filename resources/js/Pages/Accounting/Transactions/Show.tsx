...
export default function TransactionShow({kind,transaction}:{kind:'revenue'|'expense';transaction:any}) {
  ...
  return <AppShell><Head title={`Accounting — ${kind}`}/><div className="max-w-4xl mx-auto space-y-6 pb-12">
    <div className="flex justify-between gap-4"><div><h1 className="text-2xl font-bold text-[var(--text-primary)] capitalize">{kind} · {transaction.reference ?? `#${transaction.id}`}</h1><p className="text-xs text-[var(--text-secondary)] mt-1">Financial transaction lifecycle and posting evidence.</p></div><div className="flex gap-2">{transaction.status==='draft'&&<Button variant="outline" onClick={()=>router.post(`${base}/approve`)}>Approve</Button>}{transaction.status==='approved'&&<Button variant="primary" onClick={()=>router.post(`${base}/post`)}>Post to Ledger</Button>}</div></div>
